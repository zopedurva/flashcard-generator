...
const MyFlashcards = () => {
  ...
  const FLASHCARD_LIMIT = 6;
  const displayedFlashcards = showAll ? flashcards : flashcards.slice(0, FLASHCARD_LIMIT);
  ...
  return (
    <div className="container mx-auto p-8">
      ...
      {!showAll && flashcards.length > FLASHCARD_LIMIT && (
        <div className="text-center mt-6">
          <button onClick={handleShowAll} className="text-red-600">
            See All
          </button>
        </div>
      )}
